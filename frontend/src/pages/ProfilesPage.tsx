...
import { api, CatalogSyncStatus, Profile, User } from "../api/client";
...
export default function ProfilesPage() {
  ...
  const load = async () => {
    const [profiles, status] = await Promise.all([api.listProfiles(), api.catalogSyncStatus()]);
    setItems(profiles);
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-900">{t("catalog.profilesTitle")}</h2>
          <p className="text-sm text-slate-500">{t("catalog.profilesIntro")}</p>
          {syncStatus?.last_run_at && (
            <p className="text-xs text-slate-400 mt-1">
              {t("catalog.lastSync")}: {new Date(syncStatus.last_run_at).toLocaleString()}
              {" · "}
              {syncStatus.profile_count} {t("catalog.items")}
            </p>
          )}
        </div>
        {user.role === "admin" && (
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden overflow-x-auto">
        <table className="w-full text-sm min-w-[640px]">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-4 py-3 text-left">{t("catalog.profileType")}</th>
              <th className="px-4 py-3 text-left">{t("catalog.size")}</th>
              <th className="px-4 py-3 text-left">kg/m</th>
              <th className="px-4 py-3 text-left">{t("catalog.standard")}</th>
