"""The CargoPilot UN card layout: one A4 datasheet per transport entry.
...
CargoPilot's own: CargoPilot logo and wordmark in the header, no background
...
def _header(c: Canvas, page: CardPage, index: int, count: int, continued: bool) -> None:
    ...
    c.drawString(MARGIN + 42, top - 22, "CargoPilot")
...
def _footer(c: Canvas, page: CardPage, generated_at: str) -> None:
    ...
    c.drawString(MARGIN, MARGIN + FOOTER_H - 17,
                 "Generated by CargoPilot — a compilation offered as an aid; the published "
