...
- **Overzicht Materieel** — eigen materieel uit appendix-template, herkenning bij input (bijv. Demo vehicle)
...
Invulinstructies uit het tabblad **Invulinstructie** van `intern-formulier.xlsx` zijn overgenomen in `backend/app/config/dg_instructions.json`. De wizard toont deze als helptekst bij:
...
De tabel onderaan het tabblad **Overzicht Materieel** in `intern-formulier.xlsx` (SAP MATNR, specifications, afmetingen, gewicht) wordt bij eerste opstarten geladen in de database (`equipment_items`). Beheer via menu **Overzicht materieel** (admin).

Bij het plakken in de wizard herkent CargoPilot materieel op basis van SAP-code, specifications en aliassen — bijvoorbeeld `demo vehicle` → gewicht 1370 kg, afmetingen en appendix-omschrijving `VEHICLE DEMO-001`.
