...
export default function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-100 px-4">
      <form onSubmit={submit} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 w-full max-w-md space-y-4">
        <div className="text-center">
          <h1 className="text-2xl font-semibold">{t("app.name")}</h1>
          <p className="text-slate-500 text-sm mt-1">{t("app.tagline")}</p>
        </div>
        {setupWarning && <p className="text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3 text-sm">{setupWarning}</p>}
        <div>
